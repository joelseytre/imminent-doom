...
model_savefile = "tmp/model_basic_skip6.ckpt"
...
def infer_transitions(clusterer, sne_trajs, terminations):
    ...
    for start, end in traj_limits:
        ...
        idx=0
        for cluster, next_cluster in zip(clusters, next_clusters):
            idx += 1
            if cluster != next_cluster:
                print(cluster, next_cluster)
                agent_transitions[int(cluster), int(next_cluster)] += 1.
            if idx == len(clusters):
                exits[int(next_cluster)] += 1.
    ...
    return agent_transitions, exits / np.sum(exits)


def get_best_representatives(cluster_centers, memory, k=10):
...
if __name__ == '__main__':
    ...
    np.save(clustering_folder + 'transitions', transitions)
